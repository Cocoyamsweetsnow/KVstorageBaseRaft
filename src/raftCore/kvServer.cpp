...
void KvServer::ExecutePutOpOnKVDB(Op op)
{
    ...
    //    DPrintf("[KVServerExePUT----]ClientId :%d ,RequestID :%d ,Key : %v,
    //    value : %v", op.ClientId, op.RequestId, op.Key, op.Value)
    DprintfKVDB();
}
...
void KvServer::Get(const raftKVRpcProctoc::GetArgs *args, raftKVRpcProctoc::GetReply *reply)
{
    ...
    {
        //        DPrintf("[GET TIMEOUT!!!]From Client %d (Request %d) To Server %d,
        //        key %v, raftIndex %d", args.ClientId, args.RequestId, kv.me,
        //        op.Key, raftIndex)
    ...
    else
    {
        ...
        //         Value :%v", kv.me, raftIndex, op.ClientId, op.RequestId,
...
// get 与 put/append 执行的具体细节存在差异
// PutAppend 操作在收到 Raft
// 消息后执行，对应的具体函数中仅判断幂等性（是否为重复请求） get 操作在收到
// Raft 消息后执行，由于 get 是只读操作，无论是否重复都可再次执行
...
void KvServer::ReadRaftApplyCommandLoop()
{
    while (true)
    {
        ...
        DPrintf("---------------tmp-------------[func-KvServer::"
                ...
                m_me);
        // 监听其 Raft 节点已提交的所有命令，并将命令分发给对应的 RPC 处理器
...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port) : m_skipList(6)
{
    ...
    std::thread t([this, port]() -> void {
        ...
        provider.NotifyService(this->m_raftNode.get()); // todo：这里获取了原始指针，后面检查一下有没有泄露的问题
                                                        // 或者 shareptr释放的问题
    ...
    //马上向其他节点宣告自己就是leader
    //由于ReadRaftApplyCommandLoop一直不会结束，达到一直卡在这的目的
    t2.join();            
}
