    ...
    if (matchLog(args->prevlogindex(), args->prevlogterm()))
    {
        for (int i = 0; i < args->entries_size(); i++)
        {
            ...
            else
            {
                ...
                {
                    myAssert(false, format("[func-AppendEntries-rf{%d}] "
        ...
        // 领导会一次发送完所有的日志
        myAssert(getLastLogIndex() >= m_commitIndex, format("[func-AppendEntries1-rf{%d}]  rf.getLastLogIndex{%d} < "
                                                            "rf.commitIndex{%d}",
...
/*
  最新的log的logindex，即log的逻辑index。区别于log在m_logs中的物理index
...
{
    myAssert(logIndex >= m_lastSnapshotIncludeIndex, format("[func-getSlicesIndexFromLogIndex-rf{%d}]  index{%d} < "
                                                            "rf.lastSnapshotIncludeIndex{%d}",
                                                            m_me, logIndex, m_lastSnapshotIncludeIndex));

    int lastLogIndex = getLastLogIndex();

    myAssert(logIndex <= lastLogIndex, format("[func-getSlicesIndexFromLogIndex-rf{%d}]  logIndex{%d} > "
                                              "lastLogIndex{%d}",
        ...
        *appendNums = *appendNums + 1;
        ...
        if (*appendNums >= 1 + m_peers.size() / 2)
        {
            ...
            *appendNums = 0;
            ...
            myAssert(m_commitIndex <= lastLogIndex, format("[func-sendAppendEntries,rf{%d}] lastLogIndex:%d  "
                                                           "rf.commitIndex:%d\n",
...
void Raft::Snapshot(int index, std::string snapshot)
{
    ...
    if (m_lastSnapshotIncludeIndex >= index || index > m_commitIndex)
    {
        DPrintf("[func-Snapshot-rf{%d}] rejects replacing log with snapshotIndex "
                "%d as current snapshotIndex %d is larger or smaller ",
